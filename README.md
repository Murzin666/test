...
Через веб-панель (проще для повседневного использования) или через CLI.

**Веб-панель:** откройте `https://<ваш-домен>/admin`, введите значение
`ADMIN_API_KEY` из переменных окружения — дальше можно добавлять, изменять
и удалять точки прямо в браузере. Пароль банка и секрет Tilda никогда не
показываются повторно — только флаг «задан»; чтобы сменить их, впишите
новое значение, чтобы оставить прежними — оставьте поле пустым.
